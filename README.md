...
- 恢复三套悬浮窗中的换挡学习状态、F10 离合开关状态和变速箱类型显示。
- 恢复首次无离合升档自动识别序列/赛用变速箱与离合变速箱的逻辑，并持久化到车辆档案。
- 恢复 Web UI 侧栏的换挡方式徽章，补齐中英文状态文案。
- 键盘和 vJoy 均支持显式无离合换挡路径；保留现有降档补油、HUD 档位图标、动态转速刻度和透明模式修复。
- 新增变速箱快速换挡、慢速换挡及输入拦截重试回归测试。
